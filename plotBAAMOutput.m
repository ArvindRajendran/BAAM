...
% generated in this figure are similar to the plots shown in Fig. 3 and
% Fig. 5 of the original manuscript.
%
% Last modified:
...
for adsorbentNumber = 1:length(fields(outputStruct))-1
    ...
    figure('units','normalized','Position',[0.075 0.075 0.85 0.85],'Color',[1 1 1],'numbertitle', 'off');
    ...
    % Plot the purity contours for combinations of low pressure and
    % intermediate. The contour levels indicate the purity values
    subplot(2,3,3); hold on;
    [C,h] = contour(localAdsorbent.pressure.low(end,:),localAdsorbent.pressure.inter(:,1),...
        localAdsorbent.purity,[5 25 50 75 100],'r','LineWidth',1.5);
    % Generate the labels, do some graphic modifications
    clabel(C,h,'FontSize',10,'FontName','Helvetica','FontWeight','Bold')
    xlabel('Low Pressure {\it{P}}_{LOW} [bar]'); ylabel('Intermediate Pressure {\it{P}}_{INT} [bar]'); 
    title('Purity [%]');
    axH = gca; axH.LineWidth = 1; axH.FontName = 'Helvetica'; axH.FontSize = 10;
    axH.FontWeight = 'bold'; grid on; box on;
    xlim([localAdsorbent.simInfo.pressureLow localAdsorbent.simInfo.pressureLowUpperBound]);
    ylim([localAdsorbent.simInfo.pressureLow localAdsorbent.simInfo.pressureHigh]);
    
    % Plot the recovery contours for combinations of low pressure and
    % intermediate. The contour levels indicate the recovery values
    subplot(2,3,4); hold on;
    [C,h] = contour(localAdsorbent.pressure.low(end,:),localAdsorbent.pressure.inter(:,1),...
        localAdsorbent.recovery,[5 25 50 75 90 100],'b','LineWidth',1.5);
    % Generate the labels, do some graphic modifications
    clabel(C,h,'FontSize',10,'FontName','Helvetica','FontWeight','Bold')
    xlabel('Low Pressure {\it{P}}_{LOW} [bar]'); ylabel('Intermediate Pressure {\it{P}}_{INT} [bar]'); 
    title('Recovery [%]');
    ...
    % Plot the energy consumption contours for combinations of low pressure
    % and intermediate. The contour levels indicate the energy consumption 
    % values
    subplot(2,3,5); hold on;
    [C,h] = contour(localAdsorbent.pressure.low(end,:),localAdsorbent.pressure.inter(:,1),...
        localAdsorbent.energyConsumption,[50 75 90 100 250 500 1000],'m','LineWidth',1.5);
    % Generate the labels, do some graphic modifications
    clabel(C,h,'FontSize',10,'FontName','Helvetica','FontWeight','Bold')
    xlabel('Low Pressure {\it{P}}_{LOW} [bar]'); ylabel('Intermediate Pressure {\it{P}}_{INT} [bar]'); 
    title('Energy Consumption [kWh/tonne CO_2]');
    axH = gca; axH.LineWidth = 1; axH.FontName = 'Helvetica'; axH.FontSize = 10;
    axH.FontWeight = 'bold'; grid on; box on;
    xlim([localAdsorbent.simInfo.pressureLow localAdsorbent.simInfo.pressureLowUpperBound]);
    ylim([localAdsorbent.simInfo.pressureLow localAdsorbent.simInfo.pressureHigh]);
    
    % Plot the working capacity contours for combinations of low pressure
    % and intermediate. The contour levels indicate the working capacity
    % values
    subplot(2,3,6); hold on;
    [C,h] = contour(localAdsorbent.pressure.low(end,:),localAdsorbent.pressure.inter(:,1),...
        localAdsorbent.workingCapacity,[50 75 90 100 250 500 1000],'c','LineWidth',1.5);
    % Generate the labels, do some graphic modifications
    clabel(C,h,'FontSize',10,'FontName','Helvetica','FontWeight','Bold')
    xlabel('Low Pressure {\it{P}}_{LOW} [bar]'); ylabel('Intermediate Pressure {\it{P}}_{INT} [bar]'); 
    title('Working Capacity [mol CO_2/m^3 ads.]');
    axH = gca; axH.LineWidth = 1; axH.FontName = 'Helvetica'; axH.FontSize = 10;
    axH.FontWeight = 'bold'; grid on; box on;
    xlim([localAdsorbent.simInfo.pressureLow localAdsorbent.simInfo.pressureLowUpperBound]);
    ylim([localAdsorbent.simInfo.pressureLow localAdsorbent.simInfo.pressureHigh]);
